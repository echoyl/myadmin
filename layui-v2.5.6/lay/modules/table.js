...
layui.define(["laytpl", "laypage", "layer", "form", "util","sa"], function(e) {
    "use strict";
    var t = layui.$,
        i = layui.laytpl,
        a = layui.laypage,
        ...
        u = function(e, a, l, n) {
            var o = e.templet ? function() {
				//console.log(e.templet);
				//console.log(t(e.templet).html());
				if("function" == typeof e.templet)
				{
					return e.templet(l);
				}else
				{
					var first_str = e.templet.substr(0,1);
					if(first_str == '.' || first_str == '#')
					{
						//选择器
						return i(t(e.templet).html() || String(a)).render(l)
					}else
					{
						//html
						return i(e.templet || String(a)).render(l)
					}
				}
                //return "function" == typeof e.templet ? e.templet(l) : i(e.templet || String(a)).render(l)
            }() : a;
        ...
        z = ['<div class="layui-form layui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" lay-id="{{ d.data.id }}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}{{# if(d.data.height != "auto"){ }}height:{{d.data.height}}px;{{# }else{ }}height:{{d.data.height}}{{# } }}{{# } }}">', "{{# if(d.data.toolbar){ }}", '<div class="layui-table-tool">', '<div class="layui-table-tool-temp"></div>', '<div class="layui-table-tool-self"></div>', "</div>", "{{# } }}", '<div class="layui-table-box">', "{{# if(d.data.loading){ }}", '<div class="layui-table-init" style="background-color: #fff;">', '<i class="layui-icon layui-icon-loading layui-anim layui-anim-rotate layui-anim-loop"></i>', "</div>", "{{# } }}", "{{# var left, right; }}", '<div class="layui-table-header">', _(), "</div>", '<div class="layui-table-body layui-table-main">', E, "</div>", "{{# if(left){ }}", '<div class="layui-table-fixed layui-table-fixed-l">', '<div class="layui-table-header">', _({
            fixed: !0
        ...
        F = function(e) {
			...
			var search = [];
			for(var xxx in layui.router()['search'])
			{
				search.push(xxx+'='+layui.router()['search'][xxx]);
			}
			if(search.length > 0 && e.url)
			{
				//检测url中是否有? 这里还要去重一下
				if(e.url.indexOf('?') != -1)
				{
    ...
    F.prototype.config = {
            ...
            var r = function(e) {
                layui.each(t.cols, function(i, r) {
                    layui.each(r, function(i, d) {
                        ...
                        return d ? void(d.colGroup || d.hide || (e ? l && l < s && (a--, c = s) : (c = d.width || 0, /\d+%$/.test(c) ? (c = Math.floor(parseFloat(c) / 100 * o), c < s && (c = s)) : c || (d.width = c = 0, a++)), d.hide && (c = 0), n += parseFloat(c))) : void r.splice(i, 1)
                    })
                }), o > n && a && (l = (o - n) / a)
            };
            ...
            if (i.startTime = (new Date).getTime(), a.url) {
				//admin.loading();//修改table组件 添加 layadmin的 加载动画
                var r = {};
                r[l.pageName] = e, r[l.limitName] = a.limit;
                var d = t.extend(r, a.where);
                a.contentType && 0 == a.contentType.indexOf("application/json") && (d = JSON.stringify(d)), sa.request({
					type: a.method || "get",
					url: a.url,
					...
					headers: a.headers || {},
					success: function(t) {
						//admin.removeLoad();//修改table组件 添加 layadmin的 加载动画
						"function" == typeof a.parseData && (t = a.parseData(t) || t), t[n.statusName] != n.statusCode ? (i.renderForm(), i.errorView(t[n.msgName] || "返回的数据不符合规范，正确的成功状态码 (" + n.statusName + ") 应为：" + n.statusCode)) : (i.renderData(t, e, t[n.countName]), o(), a.time = (new Date).getTime() - i.startTime + " ms"), i.setColsWidth(), "function" == typeof a.done && a.done(t, e, t[n.countName])
					},
					error: function(e, t) {
						//admin.removeLoad();//修改table组件 添加 layadmin的 加载动画
                        ...
                        0 !== l.length && (r || (l[d.config.indexName] = a), c.eachCols(function(n, r) {
                            var c = r.field || n,
								...
								if((c+'').indexOf('.') > -1)
								{
									var cs = c.split('.');
									var m = l[cs[0]];
									cs.forEach(function(i,a){
										if(a > 0)
										{
											if(m && typeof m[i] != 'undefined')
											{
												m = m[i];
											}
										}
