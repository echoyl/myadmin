...
layui.define(['laytpl', 'laypage', 'layer', 'form','sa'], function(exports){
    "use strict";
    var $ = layui.$
        ,laytpl = layui.laytpl
            ...
            return ['<table cellspacing="0" cellpadding="0" border="0" class="layui-table" '
                ...
                ,'<tr>'
                    ,'{{# layui.each(item1, function(i2, item2){ }}'
                        ...
                        ,'<div style="position:relative;" class="layui-table-cell laytable-cell-'
                        ,'{{# if(item2.colspan > 1){ }}'
                        ,'group'
                        ,'{{# } else { }}'
    ...
    Class.prototype.setArea = function(){
        ...
        if(options.height){
			...
			options.height = th - that.fullHeightGap;//有的笔记本会出现高度不足的bug
        }
    ...
    Class.prototype.treeConvertShowName=function (o) {
        ...
        var temhtml='<div style="position:absolute;top:calc(50% - 14px);height: 28px;line-height: 28px;padding-left: '+
            function () {
                if(isTreeNode){
                    return '5px'
                }else{
    ...
    Class.prototype.renderTd=function (param) {
        ...
        if(options.isTree){
            if(options.treeShowName==cols.field){
				...
				obj_left = 'padding-left:'+(26 + (obj.level - 1)*19)+'px';
            }
        }
        ...
        var td = ['<td data-field="'+ field +'" '+ function(){
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
            ...
            }() +'">'+treeImgHtml+'<p style="width: auto;height: 100%;'+obj_left+'">'+ function(){
                ...
                return cols.templet ? function(){
					if("function" == typeof cols.templet)
					{
						return cols.templet(tplData);
					}else
					{
						var first_str = cols.templet.substr(0,1);
						if(first_str == '.' || first_str == '#')
						{
						//选择器
							return laytpl($(cols.templet).html() || String(content)).render(tplData)
						}else
						{
						//html
							return laytpl(cols.templet || String(content)).render(tplData)
						}
					}
                }() : content;
            }()
    ...
    Class.prototype.fullSize = function(){
		...
		console.log('父级高度：'+height);
        if(height < 135) height = 135;
        that.elem.css('height', height);
		console.log('父级高度2：'+height);
        //tbody区域高度
...
//单元格编辑
    ...
    //自动完成渲染
    table.init();
    layui.link('./dist/js/modules/treeGrid/treeGrid.css');//引入css
